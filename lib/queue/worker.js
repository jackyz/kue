...
Worker.prototype.getJob = function(fn){
  ...
  client.blpop('q:' + self.type + ':jobs',1, function(err,val) {
    // console.log("blpop "+'q:' + self.type + ':jobs  '+val);
    if (val) {
      // got a job
      self.zpop('q:jobs:' + self.type + ':inactive', function(err, id){
	// console.log("zpop "+'q:jobs:' + self.type + ':inactive'+ " ok "+id);
	if (err) return fn(err);
	if (!id) return fn();
	Job.get(id, fn);
      });
      ...
      // timeout and  next
      process.nextTick(function(){ self.getJob(fn); });
    } else {
      // console.log("quiting");
      /// ...?
    }
...
Worker.prototype.shutdown = function(fn){
  ...
  if (!this.job){
    ...
    // or wait until job done/fail
    this.once('job complete', done);
    this.once('job failed', done);
  }
};